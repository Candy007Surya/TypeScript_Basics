...
function info1(inss :string) : string
{
   return inss
}


function info2 (ins : number | string | boolean) : number| string | boolean
{
    return ins 
}

function info3 (ins : any) : any
{
    return ins ;
}

// then what is the differencet between any and type of we delcare any data at first in type it will lock it 
function info4 <Type> (ins : Type): Type
{
  return ins;
}

// this is the standard way of using it 
function info5 <T>(ins : T) : T
{
    return ins ;
}
